...
/**
 * Парсинг буфера данных
 * Преобразует входные данные в строку с помощью буфера
 * @param {Uint8Array|Buffer|string|Array<number>} data Данные для дебуферизации
 * Может быть массивом байтов, экземпляром Buffer, строкой или массивом чисел
 * @return {string} Строковое представление данных после преобразования
 */
...
/**
 * Асинхронно записывает данные в файл в формате JSON
 * @param {Object} newData Данные для записи в файл
 * @throws {Error} Если произошла ошибка при записи данных в файл
 */
...
const clearMessagesCount = (data) => {
  ...
  writeNewData(data).then((r) => r);
};

const checkUserId = (array, userId) => {
  return array.findIndex((cb) => {
    if (cb.user_id === userId) {
      return cb;
    }
  });
};

const registerEnglish = async (userId, userSignature) => {
  const OLD_DATA = await readOldData();
  const CHECK_STATUS = checkUserId(OLD_DATA, userId);
  if (CHECK_STATUS === -1) {
    const NEW_DATA = OLD_DATA.filter((callback) => {
      if (callback.name !== userSignature) {
        return callback;
      }
    });
    NEW_DATA.push({
