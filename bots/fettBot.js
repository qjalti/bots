...
BOT.start((ctx) => {
  ctx.reply(
    ...
    Markup.inlineKeyboard([
      [
        Markup.button.callback("⭐️ 1", "click_rate"),
        Markup.button.callback("⭐️ 2", "click_rate"),
        Markup.button.callback("⭐️ 3", "click_rate"),
      ],
      [
        Markup.button.callback("⭐️ 4", "click_rate"),
        Markup.button.callback("⭐️ 5", "click_rate"),
      ],
    ]),
  );
});
...
BOT.action("click_rate", (ctx) => {
  ctx.answerCbQuery();
  ctx.reply(
    "Спасибо за оценку! Если хотите что-то добавить или рассказать о проблеме — просто напишите сообщение здесь.",
  );
});
